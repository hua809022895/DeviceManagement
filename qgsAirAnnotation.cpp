...
QgsAirAnnotation::QgsAirAnnotation(QObject *parent):QObject( parent ), mMarkerSymbol( new QgsMarkerSymbol() )
{
	QgsStringMap props;
...
void QgsAirAnnotation::setVisible( bool visible )
{
  if ( mVisible == visible )
    return;
...
void QgsAirAnnotation::setHasFixedMapPosition( bool fixed )
{
...
void QgsAirAnnotation::setMapPosition( const QgsPointXY &position )
{
  mMapPosition = position;
  emit moved();
}

void QgsAirAnnotation::setMapPositionCrs( const QgsCoordinateReferenceSystem &crs )
{
  mMapPositionCrs = crs;
  emit moved();
}

void QgsAirAnnotation::setRelativePosition( QPointF position )
{
  mRelativePosition = position;
  emit moved();
}

void QgsAirAnnotation::setFrameOffsetFromReferencePoint( QPointF offset )
{
  // convert from offset in pixels at 96 dpi to mm
  setFrameOffsetFromReferencePointMm( offset / 3.7795275 );
}

QPointF QgsAirAnnotation::frameOffsetFromReferencePoint() const
{
  return mOffsetFromReferencePoint / 3.7795275;
}

void QgsAirAnnotation::setFrameOffsetFromReferencePointMm( QPointF offset )
{
...
void QgsAirAnnotation::setFrameSize( QSizeF size )
{
  // convert from size in pixels at 96 dpi to mm
  setFrameSizeMm( size / 3.7795275 );
}

QSizeF QgsAirAnnotation::frameSize() const
{
  return mFrameSize / 3.7795275;
}

void QgsAirAnnotation::setFrameSizeMm( QSizeF size )
{
...
void QgsAirAnnotation::setContentsMargin( const QgsMargins &margins )
{
  mContentsMargins = margins;
  emit appearanceChanged();
}

void QgsAirAnnotation::setFillSymbol( QgsFillSymbol *symbol )
{
  mFillSymbol.reset( symbol );
  emit appearanceChanged();
}

void QgsAirAnnotation::render(QgsRenderContext &context ) const
{
...
void QgsAirAnnotation::setMarkerSymbol( QgsMarkerSymbol *symbol )
{
  mMarkerSymbol.reset( symbol );
  emit appearanceChanged();
}

void QgsAirAnnotation::setMapLayer( QgsMapLayer *layer )
{
  mMapLayer = layer;
  emit mapLayerChanged();
}

void QgsAirAnnotation::setAssociatedFeature( const QgsFeature &feature )
{
  mFeature = feature;
}

bool QgsAirAnnotation::accept(QgsStyleEntityVisitorInterface *visitor ) const
{
...
QSizeF QgsAirAnnotation::minimumFrameSize() const
{
  return QSizeF( 0, 0 );
}

void QgsAirAnnotation::updateBalloon()
{
...
QLineF QgsAirAnnotation::segment( int index, QgsRenderContext *context ) const
{
...
void QgsAirAnnotation::drawFrame(QgsRenderContext &context ) const
{
	if (!mFillSymbol)
		return;
...
void QgsAirAnnotation::drawMarkerSymbol(QgsRenderContext &context ) const
{
	if (!context.painter())
	{
		return;
...
void QgsAirAnnotation::_writeXml( QDomElement &itemElem, QDomDocument &doc, const QgsReadWriteContext &context ) const
{
  if ( itemElem.isNull() )
  {
    return;
...
void QgsAirAnnotation::_readXml( const QDomElement &annotationElem, const QgsReadWriteContext &context )
{
...
void QgsAirAnnotation::copyCommonProperties(QgsAirAnnotation *target ) const
{
