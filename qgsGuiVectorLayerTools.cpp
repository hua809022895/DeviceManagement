...
bool QgsGuiVectorLayerTools::startEditing(QgsVectorLayer *layer ) const
{
  ...
  if ( !layer->isEditable() && !layer->readOnly() )
  {
    ...
    {
      MainWindow::instance()->m_pInfoBar->pushMessage( tr( "Start editing failed" ),
          tr( "Provider cannot be opened for editing" ), Qgis::Info, 6);
      return false;
    }
...
void QgsGuiVectorLayerTools::commitError( QgsVectorLayer *vlayer ) const
{
  ...
  QgsMessageBarItem *errorMsg = new QgsMessageBarItem(tr( "Commit errors" ),
    tr( "Could not commit changes to layer %1" ).arg(vlayer->name()),
    showMore, Qgis::Warning,  0,
    MainWindow::instance()->m_pInfoBar);
	MainWindow::instance()->m_pInfoBar->pushItem( errorMsg );
