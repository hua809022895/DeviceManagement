...
//ZJRZJR #include "qgssvgannotationdialog.h"
...
QDialog *QgsMapToolAnnotation::createItemEditor( QgsMapCanvasAnnotationItem *item )
{
    ...
    {
      //ZJRZJR QgsSvgAnnotationDialog *dialog =  new QgsSvgAnnotationDialog( item );
      //dialog->mMapCanvas = this->mCanvas;
      //return  dialog;
    }
  ...
  if ( !item || mCurrentMoveAction == QgsMapCanvasAnnotationItem::NoAction )
  {
    ...
    else
    {
      ...
      if ( annotation )
      {
        ...
        if ( svg )
        {
           annotation->setFrameSizeMm( QSizeF( 25, 25 ) );
           annotation->fillSymbol()->setOpacity(0.0);
        }else{
           annotation->fillSymbol()->setOpacity(0.5);
           annotation->setFrameSizeMm( QSizeF( 50, 25 ) );
        }
        annotation->setFrameOffsetFromReferencePointMm( QPointF(-annotation->frameSizeMm().width()/2.0f,-annotation->frameSizeMm().height()/2.0f) );
...
void QgsMapToolAnnotation::canvasMoveEvent( QgsMapMouseEvent *e )
{
  ...
  if ( e->buttons() & Qt::LeftButton )
  {
    ...
    {
       if ( static_cast<QgsSvgAnnotation*>(annotation) )
       {
           QgsPointXY mapPos = transformCanvasToAnnotation( e->snapPoint(), annotation );
           annotation->setMapPosition( mapPos );
           annotation->setRelativePosition( QPointF( e->pos().x() / mCanvas->width(),
