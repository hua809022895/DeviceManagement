...
{
  return willUseEllipsoid() ? QgsUnitTypes::AreaSquareMeters :QgsUnitTypes::distanceToAreaUnit( mCoordTransform.sourceCrs().mapUnits() );
}
...
double QgsDistanceArea::computeDistanceBearing(const QgsPointXY &p1, const QgsPointXY &p2,double *course1, double *course2 ) const
{
...
double QgsDistanceArea::computePolygonFlatArea(const QVector<QgsPointXY> &points) const
{
	...
	// 例如输入4个经纬度计算面积
	const unsigned short GroupSize =20;
	double dPt[GroupSize][2] = {{109.1227,34.3765},{109.1327,34.3865},{109.1367,34.3895},{109.1401,34.3901} };
	...
	area = dPt[0][1] * (dPt[size - 1][0] - dPt[1][0]);
	for (unsigned short index = 1; index <= size; ++index)
	{
		area += dPt[index][1] * (dPt[index - 1][0] - dPt[(index + 1) % size][0]);
	}

	area = qFabs(area / 2)*9101160000.085981;
