...
#ifndef QGSairANNOTATION_H
#define QGSairANNOTATION_H
...
class QgsAirAnnotation : public QObject
{
    Q_OBJECT
  ...
  public:
    /* Constructor for QgsAnnotation.     */
	  QgsAirAnnotation(QObject *parent SIP_TRANSFERTHIS = nullptr );

    /* Clones the annotation, returning a new copy of the annotation reflecting the annotation's current state.  */
    virtual QgsAirAnnotation *clone() const = 0 SIP_FACTORY;

    /** Returns TRUE if the annotation is visible and should be rendered. \see setVisible() */
  ...
  protected:
     ...
     */
    void copyCommonProperties(QgsAirAnnotation *target ) const;
  private:
