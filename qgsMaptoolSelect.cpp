...
void QgsMapToolSelect::canvasReleaseEvent( QgsMapMouseEvent *e )
{
  ...
  QgsVectorLayer* layer =  QgsMapToolSelectUtils::getCurrentVectorLayer( mCanvas );

  if (layer){
    int c = layer->selectedFeatures().count();
    if ( c > 0 )
    {
        MainWindow::sInstance->m_lblSelectTargetCount->setText(QString(QString::fromLocal8Bit("选中%1个")).arg(c) );
    }else{
...
void QgsMapToolSelect::populateContextMenu(QMenu *menu)
{
    menu->clear();
    QgsVectorLayer *vlayer = QgsMapToolSelectUtils::getCurrentVectorLayer( mCanvas );
    if (vlayer==g_pTargetLayer) {
            QDialogTarget target(MainWindow::instance());
            target.userinvoke = "mutiselect";
            target.setWindowTitle(QString::fromLocal8Bit("发送任务"));
            target.setMutiSelect();
            target.exec();
    }
}
