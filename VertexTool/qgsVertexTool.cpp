...
void QgsVertexTool::cadCanvasReleaseEvent( QgsMapMouseEvent *e )
{
  ...
  else if ( mSelectionRubberBand )
  {
    ...
    for ( QgsVectorLayer *vlayer : editableLayers )
    {
      ...
      //zjrzjr context.setExpressionContext( mCanvas->createExpressionContext() );
    ...
    {
      /*((MainWindow*)(gMainWindow))->m_pInfoBar->pushMessage(
...
void QgsVertexTool::showVertexEditor()  //#spellok
{
  if ( !mVertexEditor )
  {
    ...
    mVertexEditor->setMaximumWidth(200);
    if ( !gMainWindow->restoreDockWidget( mVertexEditor ) )
      gMainWindow->addDockWidget( Qt::LeftDockWidgetArea, mVertexEditor );
...
void QgsVertexTool::validationFinished()
{
  ...
  for ( ; it != mValidations.end(); ++it )
  {
    ...
    if ( validation.validator == validator )
    {
      //QgsStatusBar *sb = ((MainWindow*)gMainWindow)->m_pStatusBar;
