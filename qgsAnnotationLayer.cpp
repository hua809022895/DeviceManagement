...
void QgsAnnotationLayer::setGeometry(const QString & id, QgsPoint pos)
{
	if (!mItems.contains(id))
		return;

	QgsAnnotationItem *item = mItems.take(id);

	triggerRepaint();
}

void QgsAnnotationLayer::setAngle(const QString & id, QgsPoint pos)
{
...
QString QgsAnnotationLayer::addItem(QgsAnnotationItem *item)
{
	...
	QString			s	= QString("%1").arg(item->zIndex());
	mItems.insert(s, item);
