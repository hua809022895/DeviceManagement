...
QgsFeatureAction::QgsFeatureAction( const QString &name, QgsFeature &f, QgsVectorLayer *layer, QUuid actionId, int defaultAttr, QObject *parent )
  : QAction( name, parent ), mLayer(layer), mFeature( &f), mActionId( actionId), mIdx( defaultAttr )
  , mFeatureSaved( false)
{
}
...
QgsAttributeDialog *QgsFeatureAction::newDialog( bool cloneFeature )
{
  ...
  QgsAttributeEditorContext context(( (MainWindow*)gMainWindow)->createAttributeEditorContext() );
...
bool QgsFeatureAction::viewFeatureForm( QgsHighlight *h )
{
  ...
  QgsAttributeDialog *dialog =  ((MainWindow*)gMainWindow)->findChild<QgsAttributeDialog *>( name );
  if ( dialog )
  {
    delete h;
    dialog->raise();
...
bool QgsFeatureAction::editFeature( bool showModal )
{
  ...
  else
  {
    ...
    QgsAttributeDialog *dialog = MainWindow::instance()->findChild<QgsAttributeDialog *>( name );
    if ( dialog )
    {
      dialog->raise();
      return true;
...
bool QgsFeatureAction::addFeature( const QgsAttributeMap &defaultAttributes, bool showModal, QgsExpressionContextScope *scope SIP_TRANSFER )
{
  ...
  *mFeature = newFeature;
  ...
  else
  {
    ...
    dialog->userid = "addpoint";
    // delete the dialog when it is closed
    dialog->setAttribute( Qt::WA_DeleteOnClose );
    dialog->setMode( QgsAttributeEditorContext::AddFeatureMode );
    dialog->setEditCommandMessage( text() );
    QgsPointXY point = mFeature->geometry().asPoint();
    QString lt = QString("%1,%2").arg( point.x(),0,'g',14 ).arg( point.y(),0,'g',14 );
    MainWindow::chanegeDialogLabel( dialog, lt );
    if ( scope )
